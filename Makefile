...
.PHONY: find_python_user_dir install install_dev source clean lint test zip sonar-zip func-name build ci_build deploy
...
lint: find_python_user_dir
	...
	$(PYTHON_LOCAL_BIN)/pipenv run python3 -m flake8 --exclude .venv,build,__pycache__  --filename=**/*.py --output-file=build/$(FUNC_NAME)_lint_report.txt
...
test: find_python_user_dir
	$(PYTHON_LOCAL_BIN)/pipenv run python3 -m pytest --junitxml=build/$(FUNC_NAME)_report.xml --cov $(FUNC_NAME) --cov-report term-missing --cov-report xml:build/coverage.xml
